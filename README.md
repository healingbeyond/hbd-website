...
Controls how the deployed app is accessed:

| Value | Meaning |
| --- | --- |
| `"viktor_auth"` | Viktor-account OAuth in front of the app — **the template default**: secure, and workspace members are logged in automatically |
| `"space_auth"` | Space-level auth |
| `"public"` | No platform-level auth in front of the app |
| `null` | Inherit the platform default (used for spaces created before this descriptor existed) |
